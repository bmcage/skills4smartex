...
# Filename to save to
filename = "walking_data_shoeleft.csv"
...
#        ser_shoeright_bytes = ser_shoeright_bytes.decode("utf-8").split(',')[:-1]
        if (len(ser_shoeleft_bytes) == 3 
            #and len(ser_shoeright_bytes) == 4 
            #and len(ser_shoeaccel_bytes) == 10
            ):
            decoded_bytes = [float(x) for x in ser_shoeleft_bytes]  \
                            # + [float(x) for x in ser_shoeright_bytes]
